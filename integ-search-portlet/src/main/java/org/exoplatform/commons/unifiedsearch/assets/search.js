function initSearch() {
	...
	(function($) {
	    ...
	    ";
		// Modify by SONDN PLF August 08, 2013
		...
		String.prototype.escapeHtml = function() {
			return this.replace(/<(\/?strong)>/g, "{\$1}"). //save <strong>, </strong> as {strong}, {/strong}
		...
		function loadSiteFilter(callback) {
			$.getJSON("/rest/search/sites", function(sites) {
				...
				if (callback)
					callback(sites); //pass the available sites to be used in the callback function
			});
		}
		...
		function getSelectedSites() {
			if (SEARCH_SETTING.searchCurrentSiteOnly)
				return getUrlParam("currentSite") || parent.eXo.env.portal.portalName;
			var selectedSites = [];
		...
		function renderSearchResult(result) {
			...
			switch (result.type) {
				case "event":
					...
					avatar = EVENT_AVATAR_TEMPLATE.replace(/%{month}/g, date[1]).replace(/%{date}/g, date[2]);
					break;
					
				case "task":
					// Modify by SON PLF at 2013.05.08
				...
				case "file":
					var cssClasses = $.map(result.fileType.split(/\s+/g), function(type) {
						return "uiIcon64x64" + type
					}).join(" ");
			...
			var html = SEARCH_RESULT_TEMPLATE.replace(/%{type}/g, result.type).replace(/%{url}/g, result.url).replace(/%{title}/g, (result.title || "").highlight(terms)).replace(/%{excerpt}/g, (result.excerpt || "").escapeHtml().highlight(terms)).replace(/%{detail}/g, (result.detail || "").highlight(terms)).replace(/%{avatar}/g, avatar).replace(/%{rating}/g, rating);
		...
		$("#btnShowMore").click(function() {
			...
			if (remaining.length < LIMIT) {
				...
				getFromServer(function() {
					...
					$("#searchPage").animate({
						scrollTop : $("#resultPage")[0].scrollHeight
					}, "slow");
				});
				return;
			}
